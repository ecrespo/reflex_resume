...
- `web/web.py` — creates `rx.App`, global `<head>` (Inter font, `.prose` CSS for rendered posts), defines `index` and `dev_stats_page`, and registers all routes: `/`, `/dev-stats`, `/blog`, `/blog/archives`, `/blog/categories`, `/blog/tags`, plus one static route per post.
...
- **Dev Stats page** (`/dev-stats`): an EDA of the GitHub account rendered with `reflex_rosencharts`. Figures are static constants in `web/dev_stats_data.py` (with `DATA_AS_OF`) — never a render-time GitHub API call, since the frontend is exported statically and the unauthenticated API is capped at 60 req/hour. `web/states/dev_stats_state.py` exposes them as plain vars with static defaults (Reflex embeds initial state in the page, so the charts render even if the backend is unreachable), and `web/components/dev_stats_sections.py` builds every section through one `chart_figure()` helper that also emits a screen-reader-only table. One section is live rather than frozen: the contributions calendar (`reflex-react-github-calendar`) fetches from `github-contributions-api.jogruber.de` in the visitor's browser. Spec: `specs/dev-stats-page.md`.
- **Vite dev dependency pre-bundling**: `VitePrebundlePlugin` in `rxconfig.py` adds `optimizeDeps.include` (`d3`, `react-github-calendar`, `@knight-lab/timelinejs`) to the generated `.web/vite.config.js`. These NoSSR components import their packages dynamically, so without it `reflex run` re-optimizes mid-session and fails with "error loading dynamically imported module …/.vite/deps/d3.js?v=…" (charts stay blank). Add any new dynamically-loaded npm package there; after changing it, restart `reflex run` (and `rm -rf .web/node_modules/.vite` if the error persists).
- **Blog** (`web/blog/`):
